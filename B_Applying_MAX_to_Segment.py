...
class SegmentTree:
    ...
    def add(self,node,i,j,val,left,right):
        ...
        if i<=left and right<=j:
            self.values[node] = max(self.values[node],val)
            return
    ...
    def find(self,node,idx,left,right):
        ...
        mid = (left+right)//2
        if idx < mid:
            return max(self.find(node*2+1,idx,left,mid),self.values[node])
        return max(self.find(node*2+2,idx,mid,right),self.values[node])
tree = SegmentTree(n)
